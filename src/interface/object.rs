...
fn extract_data<'a>(input_object: &'a PyAny, attribute: &'a str,
...
pub fn object_interface<'a>(input_object: &'a PyAny) -> PyResult<&'a PyAny> {
    ...
    input_object.setattr(
        "number_results",
        output_dict.get_item( "NUMBER RESULT").unwrap()).unwrap();
    input_object.setattr(
        "numbers_results",
        output_dict.get_item( "NUMBERS RESULT").unwrap()).unwrap();
